      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-8 border-b border-[var(--border-color)] pb-6">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-blue-600/10 border border-blue-500/20 text-blue-500">
        ...
        <div className="grid grid-cols-1 gap-6">
          {categories.map((cat, idx) => (
            <div key={idx} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 bg-[var(--bg-hover)]/40 border-b border-[var(--border-color)]">
                <h2 className="text-xs font-bold uppercase tracking-wider text-[var(--text-sub)]">{cat.title}</h2>
              </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {item.keys.map((key, keyIdx) => (
                        <React.Fragment key={keyIdx}>
                          ...
                          {keyIdx < item.keys.length - 1 && <span className="text-xs text-[var(--text-muted)] font-semibold">+</span>}
                        </React.Fragment>
                      ))}
                      {item.secondaryKeys && (
                        <>
                          ...
                          {item.secondaryKeys.map((key, keyIdx) => (
                            <React.Fragment key={keyIdx}>
                              ...
                              {keyIdx < item.secondaryKeys!.length - 1 && <span className="text-xs text-[var(--text-muted)] font-semibold">+</span>}
                            </React.Fragment>
                          ))}
                        </>
                      )}
        ...
        <div className="mt-8 p-5 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border border-blue-500/20 rounded-2xl flex gap-4 text-sm shadow-lg shadow-blue-500/2">
          <div className="p-2 rounded-xl bg-blue-500/20 text-blue-500 self-start">
            <Keyboard className="w-5 h-5" />
          </div>
          <div>
            <h4 className="font-bold text-[var(--text-main)] mb-1">Keyboard-only Navigation Tips</h4>
